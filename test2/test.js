...
let product;
  if (data.intents[i].tag === "single") {
     product = compareRandom(data.intents[i].patterns,data.intents[i].responses,input);
    addChat(input, product);
  }else if (data.intents[i].tag === "all") {
    product = compareRandom(data.intents[i].patterns,data.intents[i].responses,input);
    addChatList(input,product);
  }else{
...
function compareRandom(promptsArray, repliesArray, string) {
  ...
  function compareList(promptsArray, repliesArray, string) {
    let text = string.toLowerCase().replace(/[^\w\s]/gi, "").replace(/[\d]/gi, "").trim();
    text = text
    .replace(/ a /g, " ")   // 'tell me a story' -> 'tell me story'
    .replace(/i feel /g, "")
